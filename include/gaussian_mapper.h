...
public:
    struct MergeParams {
        ...
        float grad_th = 0.001f;
        int from_iter = 1000;
        int until_iter = 30000;
        int interval = 1000;
        int kf_win_size = 20;
        float md_dist_th = 4.0f;
        bool enable_LBFGS_opt = true;
    } merge_params;

    bool doDensify = true;
    bool mbdoGaussianMerge_ = true;  // do gaussian merge
    std::size_t lastRenderKFId = 0;
    void mergeGaussiansInVoxel(torch::Tensor& clipped_mask);
    void clearGaussiansInVoxel();

    std::mutex mutex_merge_;
    int do_merge_cnt = 0;
    int local_min_kfID = 0;
    int local_max_kfID = 0;
};