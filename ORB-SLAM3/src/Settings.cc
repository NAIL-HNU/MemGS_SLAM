...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        readGridParameters(fSettings);
        cout << "\t-Loaded grid parameters" << endl;
    ...
    void Settings::readGridParameters(cv::FileStorage& fSettings) {
        bool found;
        int gridSize = readParameter<int>(fSettings,"Grid.size",found,false);
        int numThr = readParameter<int>(fSettings,"Grid.numThr",found,false);
        bool enabled = (bool) readParameter<int>(fSettings,"Grid.enabled",found,false);
        if(found){
            static_grid.enabled = enabled;
            static_grid.size = gridSize;
            static_grid.numThr = numThr;
        } else {
            std::cerr << "Grid parameters not found, using default values." << std::endl;
