    ...
    std::vector<float> &pointsLocal)
{
    ...
    if (static_grid.enabled && !this->mbAddTexturelessPoints && this->mnId > 10) {
        this->AddTextureLessPoints(this->imgLeftRGB, pixelsUndist, pointsLocal);
        this->mbAddTexturelessPoints = true;
    }
}
    ...
    for(int y = 0; y < num_grid_y; y++){
        for(int x = 0; x < num_grid_x; x++){
            if(no_feature_areas.at<uchar>(y, x)){
                ...
                y_end = std::min((y + 1) * grid_size - edge_margin, ImGray.rows);
                if ( x_start >= x_end || y_start >= y_end) continue;  // Skip the cell if it is too small
                if (x_end -x_start < grid_size-30 || y_end - y_start < grid_size -30) continue;
                ...
                for (int i=0; i < cnt_for; i++) {
                    x_new = dis_x(gen), y_new = dis_y(gen);
                    if(mask.at<uchar>(y_new, x_new) == 0){
                        pixelsUndist.emplace_back(x_new);
                        pixelsUndist.emplace_back(y_new);
                        
                        // if RGB-D, then get depth value
                        float depth = -1.0f;
                        if (!this->imgAuxiliary.empty() && y_new < this->imgAuxiliary.rows && x_new < this->imgAuxiliary.cols) {
                            depth = this->imgAuxiliary.at<float>(y_new, x_new);
                        }

                        if (depth > 0 && std::isfinite(depth)) {
                            Eigen::Vector3f pt_camera;
                            pt_camera.z() = depth;
                            pt_camera.x() = (x_new - cx) * depth * invfx;
                            pt_camera.y() = (y_new - cy) * depth * invfy;
                            pointsLocal.emplace_back(pt_camera.x());
                            pointsLocal.emplace_back(pt_camera.y());
                            pointsLocal.emplace_back(pt_camera.z());
                        } else {
                            pointsLocal.emplace_back(0.0f);
                            pointsLocal.emplace_back(0.0f);
