...
public:
    ...
    // add func
    std::vector<std::size_t> getAllKFsId() const {
        std::unique_lock<std::mutex> lock(mMutexKeyFrames);
        std::vector<std::size_t> vKFIds;
        vKFIds.reserve(mvAssociatedKeyFrames.size());
        for (const auto& kfit : mvAssociatedKeyFrames) {
            vKFIds.emplace_back(std::get<0>(kfit));
        }
        return vKFIds;
    }
...
protected:
    ...
    std::vector<std::tuple<
        ...
        double/*time stamp*/>> mvAssociatedKeyFrames;
