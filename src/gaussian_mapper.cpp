      ...
      training_report_interval_(0)
{
    ...
    gaussians_->merge_params.resolution = merge_params.resolution;
    gaussians_->merge_params.verbose = merge_params.verbose;
    gaussians_->merge_params.enable_LBFGS_opt = merge_params.enable_LBFGS_opt;
...
void GaussianMapper::readConfigFromFile(std::filesystem::path cfg_path)
{
    ...
    // Merge Parameters
    merge_params.enabled =
        (settings_file["Merge.enabled"].operator int()) != 0;
    merge_params.verbose =
        (settings_file["Merge.verbose"].operator int()) != 0;
    merge_params.resolution =
        settings_file["Merge.resolution"].operator float();
    merge_params.grad_th = 
        settings_file["Merge.clipped_grad_th"].operator float();
    merge_params.from_iter = 
        settings_file["Merge.from_iter"].operator int();
    merge_params.until_iter = 
        settings_file["Merge.until_iter"].operator int();
    merge_params.interval = 
        settings_file["Merge.interval"].operator int();
    merge_params.kf_win_size = 
        settings_file["Merge.kf_window_size"].operator int();
    merge_params.md_dist_th =
        settings_file["Merge.md_dist_th"].operator float();
    general_utils::md_thr = merge_params.md_dist_th;
    merge_params.enable_LBFGS_opt =
        (settings_file["Merge.enable_LBFGS_opt"].operator int()) != 0;
    // When LBFGS optimization is disabled, set the interval to match the densification interval for real-time merging.
    if (merge_params.enabled && !merge_params.enable_LBFGS_opt) {
        merge_params.interval = opt_params_.densification_interval_;
    }
        ...
        if (hasMetInitialMappingConditions()) {
            ...
            std::vector<ORB_SLAM3::MapPoint*> vpMPs;
            {
                ...
                for (const auto& pKF : vpKFs){
                    ...
                    try {
                        ...
                        new_kf->timestamp_ = pKF->mTimeStamp;
                    ...
                    this->lastRenderKFId = pKF->mnId;
                    this->local_max_kfID = pKF->mnId;
                }
            }
            ...
            for (auto& kfit : scene_->keyframes()) {
                ...
                if (device_type_ == torch::kCUDA) {
                    ...
                    if (this->sensor_type_ == RGBD) {
                        cv::cuda::GpuMat depth_img_gpu;
                        depth_img_gpu.upload(pkf->img_auxiliary_undist_);
                        pkf->gaus_pyramid_depth_image_.resize(num_gaus_pyramid_sub_levels_);
                        for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                            cv::cuda::GpuMat depth_img_resized;
                            cv::cuda::resize(depth_img_gpu, depth_img_resized,
                                            cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                            pkf->gaus_pyramid_depth_image_[l] =
                                tensor_utils::cvGpuMat2TorchTensor_Float32(depth_img_resized);
                        }
                    }
                }
                else {
                    ...
                    if (this->sensor_type_ == RGBD) {
                        pkf->gaus_pyramid_depth_image_.resize(num_gaus_pyramid_sub_levels_);
                        for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                            cv::Mat depth_img_resized;
                            cv::resize(pkf->img_auxiliary_undist_, depth_img_resized,
                                    cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                            pkf->gaus_pyramid_depth_image_[l] =
                                tensor_utils::cvMat2TorchTensor_Float32(depth_img_resized, device_type_);
                        }
                    }
                }
            }
            ...
            // Prepare for training
            {
                ...
                gaussians_->createFromPcd(scene_->cached_point_cloud_, scene_->cameras_extent_, this->lastRenderKFId);
                std::unique_lock<std::mutex> lock(mutex_settings_);
...
void GaussianMapper::trainForOneIteration()
{
    ...
    torch::Tensor gt_image, mask, gt_img_depth;
    ...
    if (training_level == num_gaus_pyramid_sub_levels_) {
        ...
        if (this->sensor_type_ == RGBD) {
            gt_img_depth = viewpoint_cam->depth_image_.cuda();
        }
    }
    else {
        ...
        if (this->sensor_type_ == RGBD) {
            gt_img_depth = viewpoint_cam->gaus_pyramid_depth_image_[training_level].cuda();
        }
    }
    ...
    torch::Tensor scaling = gaussians_->getScalingActivation();
    torch::Tensor isotropic_loss = torch::abs(scaling - scaling.mean(/* dim */1, /* keepdim */true)); 
    loss += isotropic_loss.mean();

    // if (this->sensor_type_ == RGBD) { 
    //     torch::Tensor valid_mask = (gt_img_depth > RGBD_min_depth_) & (gt_img_depth < RGBD_max_depth_) & torch::isfinite(gt_img_depth);

    //     torch::Tensor valid_cnt = valid_mask.sum();

    //     auto rendered_depth = depth * mask;  // [3, H, W]
    //     rendered_depth = rendered_depth.index({0, "..."}); // [H, W]

    //     torch::Tensor depth_diff = torch::abs(rendered_depth - gt_img_depth);
    //     depth_diff = depth_diff.masked_fill(~valid_mask, 0.0f);

    //     torch::Tensor L1_depth = depth_diff.sum() / (valid_cnt + 1e-8);
    //     loss += L1_depth; 
    // }
...

    {
        ...
        if (keyframe_record_interval_ &&
            ...
            recordKeyframeRendered(masked_image, gt_image, viewpoint_cam->timestamp_, result_dir_, result_dir_, result_dir_);
        ...
        if (getIteration() < opt_params_.densify_until_iter_) {
            ...
            if (!merge_params.enabled || doDensify) {
                // Densify and Prune
            ...
            } else if (merge_params.enabled && !doDensify && getIteration() < merge_params.until_iter) {
                if (getIteration() >= merge_params.from_iter && getIteration() % merge_params.interval == 0) {
                    torch::Tensor clipped_mask = gaussians_->unique_kfID >= local_max_kfID - merge_params.kf_win_size;
                    clipped_mask = clipped_mask.logical_and(gaussians_->unique_kfID <= local_max_kfID);
                    auto clipped_sum = clipped_mask.sum();
                    auto normalized_grads = gaussians_->xyz_gradient_accum_ / gaussians_->denom_;
                    normalized_grads.index_put_({normalized_grads.isnan()}, 0.0f);
                    auto grad_mask = torch::where(torch::frobenius_norm(normalized_grads, /* dim */-1) < merge_params.grad_th, true, false ).to(clipped_mask.device());
                    auto grad_sum = grad_mask.sum();
                    auto filtered_mask = torch::logical_and(clipped_mask, grad_mask);
                    auto union_sum = filtered_mask.sum();
                    
                    if (filtered_mask.any().item<bool>()) {
                        torch::Tensor clipped_xyz = gaussians_->getXYZ().index({filtered_mask});
                        clipped_xyz = clipped_xyz.cpu().contiguous();
                        auto xyz_accessor = clipped_xyz.accessor<float, 2>();
                        for (std::size_t i = 0; i < clipped_xyz.size(0); ++i) {
                            float x = xyz_accessor[i][0], y = xyz_accessor[i][1], z = xyz_accessor[i][2];
                            gaussians_->insertPointsToVoxel(x, y, z, gaussians_->pts_num);
                            gaussians_->pts_num++;
                        }
                        mergeGaussiansInVoxel(filtered_mask);
                        clearGaussiansInVoxel();
                        do_merge_cnt++;
                    } 
                }
            }
            if (merge_params.enabled && getIteration() % densifyInterval() == 0 ) { this->doDensify = !this->doDensify; }
            else if (!merge_params.enabled) {this->doDensify = true; }
...
void GaussianMapper::combineMappingOperations()
{
    ...
    while (pSLAM_->getAtlas()->hasMappingOperation()) {
        ...
        if (!localKFIds.empty()) {
            ...
            this->local_min_kfID = *min_max.first;
            this->local_max_kfID = *min_max.second;
        } 
        ...
        case ORB_SLAM3::MappingOperation::OprType::LocalMappingBA:
        {
            ...
            for (auto& kf : associated_kfs) {
                ...
                if (pkf) {
                    ...
                    if (kfid > this->lastRenderKFId + 2) {
                        this->lastRenderKFId = kfid;  // 更新最近渲染的关键帧ID
                    }
            ...
            if (initial_mapped_ && points.size() >= 30) {
                torch::NoGradGuard no_grad;
                std::unique_lock<std::mutex> lock_render(mutex_render_);
                gaussians_->increasePcd(points, colors, getIteration(), this->local_max_kfID);
            }
        }
        break;
...
// savePly(result_dir_ / (std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_) + "_1_after_inactive_geo_densify"));
    ...
    if (depth_cached_ >= max_depth_cached_) {
        ...
        // Add new points to the model
        std::unique_lock<std::mutex> lock_render(mutex_render_);
        gaussians_->increasePcd(depth_cache_points_, depth_cache_colors_, getIteration(), pkf->fid_);
    }
        ...
        std::string name_suffix)
{
    if (record_rendered_image_) {
        ...
        cv::imwrite(result_img_dir / (std::to_string(timestamp) + ".png"), image_cv);
    }
    ...
    if (record_ground_truth_image_) {
        ...
        cv::imwrite(result_gt_dir / (std::to_string(timestamp) + name_suffix + ".png"), gt_image_cv);
    }
    ...
    if (record_loss_image_) {
        ...
        cv::imwrite(result_loss_dir / (std::to_string(timestamp) + ".png"), loss_image_cv);
    }
}
    ...
    const bool main_vision)
{
    ...
    std::tuple<at::Tensor/* rgb */, at::Tensor/* viewspace_points */, at::Tensor/* visibility_filter */, at::Tensor/* radii */, torch::Tensor/* depth */, torch::Tensor/* opacity */, torch::Tensor/* n_touched */> render_pkg;
    {
        std::unique_lock<std::mutex> lock_render(mutex_render_);
        // Render
    ...
    std::string name_suffix)
{
    ...
    recordKeyframeRendered(masked_image, gt_image, pkf->timestamp_, result_img_dir, result_gt_dir, result_loss_dir, name_suffix);
}
    ...
    std::string name_suffix)
{
    ...
    std::filesystem::path image_dir = result_dir / "image_rendered";
    if (record_rendered_image_)
    ...
    std::filesystem::path kf_pose_path = result_dir / "gaussian_keyframe_poses.txt";
    std::ofstream out_kf_poses(kf_pose_path);
    out_kf_poses << "##[Gaussian Mapper]Keyframe poses: timestamp, tx, ty, tz, qx, qy, qz, qw" << std::endl;

    std::size_t nkfs = scene_->keyframes().size();
    auto kfit = scene_->keyframes().begin();
    float dssim, psnr, psnr_gs;
    double render_time;
    std::vector<double> avg_psnr, avg_psnr_gs; 
    for (std::size_t i = 0; i < nkfs; ++i) {
        ...
        avg_psnr.emplace_back(psnr);
        avg_psnr_gs.emplace_back(psnr_gs);
        
        // 保存相机位姿
        Sophus::SE3f Tcw = (*kfit).second->getPosef();
        Sophus::SE3f Twc = Tcw.inverse();
        Eigen::Quaternionf q = Twc.unit_quaternion();
        Eigen::Vector3f t = Twc.translation();
        out_kf_poses << std::fixed << std::setprecision(6)
                     << (*kfit).second->timestamp_ << " "
                     << t.x() << " " << t.y() << " " << t.z() << " "
...
void GaussianMapper::keyframesToJson(std::filesystem::path result_dir)
{
    ...
    for (const auto& kfit : scene_->keyframes()) {
        ...
        json_kf["img_name"] = std::to_string(pkf->timestamp_); //(std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_));
...
void GaussianMapper::mergeGaussiansInVoxel(torch::Tensor& filtered_mask) {
    gaussians_->traverseVoxel();
    if (gaussians_->merged_pts_idxs.size() > 0) {
        std::unique_lock<std::mutex> lock_render(mutex_merge_);
        gaussians_->mergeGaussiansByMDInCUDA(filtered_mask);
    } 
}

void GaussianMapper::clearGaussiansInVoxel() {
    gaussians_->voxel_map.clear();
    gaussians_->merged_pts_idxs.clear();
    gaussians_->reserved_pt_idxs.clear();
    gaussians_->vox_pts_num.clear();
    gaussians_->pts_num = 0;
}