    ...
    const int kf_id)
{
    ...
    this->unique_kfID = torch::zeros({this->getXYZ().size(0)}, torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU))*kf_id;
}

void GaussianModel::increasePcd(std::vector<float> points, std::vector<float> colors, const int iteration, const int kf_id)
{
...
// std::cout << "increasePcd(umap) preparation time: " << time << " ms" <<std::endl;
    ...
    torch::Tensor new_kf_ids = torch::ones({new_point_cloud.size(0)},torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU))*kf_id;
...
void GaussianModel::increasePcd(torch::Tensor& new_point_cloud, torch::Tensor& new_colors, const int iteration, const int kf_id)
{
// auto time1 = std::chrono::steady_clock::now();
...
// std::cout << "increasePcd(tensor) preparation time: " << time << " ms" <<std::endl;
    torch::Tensor new_kf_ids = torch::zeros( {new_point_cloud.size(0)}, torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU))*kf_id;
...
void GaussianModel::prunePoints(torch::Tensor& mask)
{
    ...
    this->unique_kfID = this->unique_kfID.index({valid_points_mask.cpu()});
}
    ...
    torch::Tensor& new_kf_ids)
{
    ...
    this->denom_ = torch::zeros({this->getXYZ().size(0), 1}, torch::TensorOptions().device(device_type_));
    this->max_radii2D_ = torch::zeros({this->getXYZ().size(0)}, torch::TensorOptions().device(device_type_));
    if (new_kf_ids.defined()) {
        this->unique_kfID = torch::cat({this->unique_kfID, new_kf_ids}, /*dim=*/0).to(torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU));
    }
}
    ...
    int N)
{
    ...
    torch::Tensor new_kf_id = this->unique_kfID.index({selected_pts_mask.cpu()}).repeat({N}).to(torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU));
    ...
    float scene_extent)
{
    ...
    torch::Tensor new_kf_id = this->unique_kfID.index({selected_pts_mask.cpu()}).to(torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU));
...
void GaussianModel::loadPly(std::filesystem::path ply_path)
{
    ...
    this->unique_kfID = torch::zeros({num_points}, torch::TensorOptions().dtype(torch::kInt16).device(torch::kCPU));
}
...
torch::Tensor GaussianModel::getActualCovarianceActivation(int scaling_modifier) {
    // build_rotation
    auto r = this->rotation_;
    auto R = general_utils::build_rotation(r);
...
void GaussianModel::calculateMDInCUDA(torch::Tensor& merged_idxs_tmp, torch::Tensor& clipped_mask, torch::Tensor& merged_MD) {

    torch::Tensor clipped_xyz = this->xyz_.index({clipped_mask}); 
    torch::Tensor clipped_cov3D_inv = this->getActualCovarianceActivation().index({clipped_mask}).inverse();  

    CallMergeGaussianInCUDA::MergeGaussian::calculateMDInCUDA(clipped_xyz, clipped_cov3D_inv, merged_idxs_tmp, merged_MD); 
}

void GaussianModel::MergeGaussiansInCUDA(torch::Tensor& reserved_idxs, torch::Tensor& merged_idxs, torch::Tensor& clipped_mask)
{
    torch::Tensor clipped_xyz = this->xyz_.index({clipped_mask});
    torch::Tensor clipped_features_dc = this->features_dc_.index({clipped_mask});
    torch::Tensor clipped_features_rest = this->features_rest_.index({clipped_mask});
    torch::Tensor clipped_opacities = this->opacity_.index({clipped_mask});
    torch::Tensor clipped_scaling = this->scaling_.index({clipped_mask});
    torch::Tensor clipped_rotation = this->rotation_.index({clipped_mask});
    torch::Tensor clipped_exist_since_iter = this->exist_since_iter_.index({clipped_mask});
    torch::Tensor clipped_kf_ids = this->unique_kfID.index({clipped_mask.cpu()});

    torch::Tensor reserved_xyz = clipped_xyz.index({reserved_idxs});
    torch::Tensor reserved_features_dc = clipped_features_dc.index({reserved_idxs});
    torch::Tensor reserved_features_rest = clipped_features_rest.index({reserved_idxs});
    torch::Tensor reserved_opacities = clipped_opacities.index({reserved_idxs});
    torch::Tensor reserved_scaling = clipped_scaling.index({reserved_idxs});
    torch::Tensor reserved_rotation = clipped_rotation.index({reserved_idxs});
    torch::Tensor reserved_exist_since_iter = clipped_exist_since_iter.index({reserved_idxs});
    torch::Tensor reserved_kf_ids = clipped_kf_ids.index({reserved_idxs.cpu()});

    int size = merged_idxs.size(0);
    torch::Tensor merged_A_idxs = merged_idxs.index({torch::indexing::Slice(), 0});
    torch::Tensor merged_B_idxs = merged_idxs.index({torch::indexing::Slice(), 1});

    // merged_xyz: x = (C1^-1 + C2^-1)^-1 * (C1^-1 * x1 + C2^-1 * x2)
    torch::Tensor clipped_cov3D = this->getActualCovarianceActivation().index({clipped_mask});      
    torch::Tensor merged_A_Sigma = clipped_cov3D.index({merged_A_idxs, torch::indexing::Slice(), torch::indexing::Slice()});  
    torch::Tensor merged_B_Sigma = clipped_cov3D.index({merged_B_idxs, torch::indexing::Slice(), torch::indexing::Slice()});  

    torch::Tensor merged_A_xyz = clipped_xyz.index({merged_A_idxs});                                
    torch::Tensor merged_B_xyz = clipped_xyz.index({merged_B_idxs});                                

    torch::Tensor merged_SigmaA_inv = merged_A_Sigma.inverse();                                     
    torch::Tensor merged_SigmaB_inv = merged_B_Sigma.inverse();                                     
    torch::Tensor merged_cov3D_inv = (merged_SigmaA_inv + merged_SigmaB_inv).inverse();             
    torch::Tensor xyzA = torch::bmm(merged_SigmaA_inv, merged_A_xyz.unsqueeze(2)).squeeze(2);       
    torch::Tensor xyzB = torch::bmm(merged_SigmaB_inv, merged_B_xyz.unsqueeze(2)).squeeze(2);       
    torch::Tensor merged_xyz = torch::bmm(merged_cov3D_inv, (xyzA + xyzB).unsqueeze(2)).squeeze(2);

    torch::Tensor merged_idx_max = std::get<0>(torch::max(merged_idxs, /*dim=*/1));                 
    torch::Tensor merged_features_dc = clipped_features_dc.index({merged_idx_max});
    torch::Tensor merged_features_rest = clipped_features_rest.index({merged_idx_max});
    torch::Tensor merged_opacities = clipped_opacities.index({merged_idx_max});                     

    torch::Tensor merged_rotation = torch::empty({size, 4}, torch::TensorOptions().dtype(torch::kFloat32).device(device_type_));
    torch::Tensor merged_scaling = torch::empty({size, 3}, torch::TensorOptions().dtype(torch::kFloat32).device(device_type_));
    torch::Tensor merged_A_scaling = torch::exp(clipped_scaling.index({merged_A_idxs}));
    torch::Tensor merged_B_scaling = torch::exp(clipped_scaling.index({merged_B_idxs}));
    torch::Tensor init_scaling = (merged_A_scaling + merged_B_scaling) / 2.0f;

    torch::Tensor merged_A_quat = torch::nn::functional::normalize(clipped_rotation.index({merged_A_idxs}));
    torch::Tensor merged_B_quat = torch::nn::functional::normalize(clipped_rotation.index({merged_B_idxs}));
    torch::Tensor init_rotation = CallMergeGaussianInCUDA::MergeGaussian::slerp(merged_A_quat, merged_B_quat, 0.5f);

    torch::Tensor meanAScaling = merged_A_scaling.mean(-1);
    torch::Tensor meanBScaling = merged_B_scaling.mean(-1);
    float iso_threash = 1e-4f; // 0.01f is a good value for iso threshold, it can be tuned
    torch::Tensor scale_mask = (meanAScaling < iso_threash) & (meanBScaling < iso_threash);              
    torch::Tensor quat_mask = torch::abs(merged_A_quat.index({torch::indexing::Slice(), 0}) - 1.0) < iso_threash &
                              torch::abs(merged_B_quat.index({torch::indexing::Slice(), 0}) - 1.0) < iso_threash;
    torch::Tensor iso_mask = scale_mask.unsqueeze(-1) | quat_mask.unsqueeze(-1);      

    {
        merged_scaling = torch::where(iso_mask, init_scaling, merged_scaling);
        merged_rotation = torch::where(iso_mask, init_rotation, merged_rotation);
    }
   
    torch::Tensor no_iso_mask = ~(iso_mask.squeeze(-1));
    if ((no_iso_mask.sum() < 2).item<bool>()) { return; } else {
        init_rotation = init_rotation.index({no_iso_mask, torch::indexing::Slice()});
        init_scaling = init_scaling.index({no_iso_mask, torch::indexing::Slice()});

        torch::Tensor ret_merged_rotation, ret_merged_scaling;
        if (merge_params.enable_LBFGS_opt) {
            merged_A_Sigma = merged_A_Sigma.index({no_iso_mask, torch::indexing::Slice(), torch::indexing::Slice()});
            merged_B_Sigma = merged_B_Sigma.index({no_iso_mask, torch::indexing::Slice(), torch::indexing::Slice()});
            std::tie(ret_merged_rotation, ret_merged_scaling) = 
                        CallMergeGaussianInCUDA::MergeGaussian::optimizeRotationAndScaleLBFGS( init_rotation, init_scaling, merged_A_Sigma, merged_B_Sigma);
        } else {
            ret_merged_rotation = init_rotation;
            ret_merged_scaling = init_scaling;
        }
        torch::cuda::synchronize();  
        merged_rotation.index_put_({no_iso_mask, torch::indexing::Slice()}, ret_merged_rotation);
        merged_scaling.index_put_({no_iso_mask, torch::indexing::Slice()}, ret_merged_scaling);
    }

    torch::Tensor merged_exist_since_iter = clipped_exist_since_iter.index({merged_idx_max});
    torch::Tensor merged_kf_ids = clipped_kf_ids.index({merged_idx_max.cpu()});

    int before_merge_size = clipped_xyz.size(0);
    clipped_xyz = torch::cat({reserved_xyz, merged_xyz}, /*dim=*/0);
    clipped_features_dc = torch::cat({reserved_features_dc, merged_features_dc}, /*dim=*/0);
    clipped_features_rest = torch::cat({reserved_features_rest, merged_features_rest}, /*dim=*/0);
    clipped_opacities = torch::cat({reserved_opacities, merged_opacities}, /*dim=*/0);
    merged_scaling = torch::log(merged_scaling);
    clipped_scaling = torch::cat({reserved_scaling, merged_scaling}, /*dim=*/0);
    clipped_rotation = torch::cat({reserved_rotation, merged_rotation}, /*dim=*/0);
    clipped_exist_since_iter = torch::cat({reserved_exist_since_iter, merged_exist_since_iter}, /*dim=*/0);
    clipped_kf_ids = torch::cat({reserved_kf_ids, merged_kf_ids}, /*dim=*/0);

    this->prunePoints(clipped_mask);
    densificationPostfix(clipped_xyz, clipped_features_dc, clipped_features_rest, clipped_opacities, clipped_scaling, clipped_rotation,
                            clipped_exist_since_iter, clipped_kf_ids);
    c10::cuda::CUDACachingAllocator::emptyCache(); 
}

void GaussianModel::mergeGaussiansByMDInCUDA(torch::Tensor& clipped_mask, bool merge_still_reserved) {
    int row = merged_pts_idxs.size() / 2;                                                           
    torch::Tensor merged_MD = torch::zeros({row, 1}, torch::TensorOptions().dtype(torch::kFloat32).device(device_type_));                                   
    torch::Tensor merged_idxs_tmp = torch::from_blob(merged_pts_idxs.data(), {row, 2}, torch::TensorOptions().dtype(torch::kInt32)).to(device_type_); 
    this->calculateMDInCUDA(merged_idxs_tmp, clipped_mask, merged_MD);                              
    torch::cuda::synchronize();                                                                     
    
    int reserved_pts_num = reserved_pt_idxs.size();                                                 
    torch::Tensor reserved_idxs = torch::from_blob(reserved_pt_idxs.data(), {reserved_pts_num}, torch::TensorOptions().dtype(torch::kInt32)).to(device_type_);
    int total_vox_num = vox_pts_num.size();
    int merged_vox_num = total_vox_num - reserved_pts_num;                                          

    torch::Tensor merged_idxs = torch::empty({merged_vox_num, 2}, torch::TensorOptions().dtype(torch::kInt32).device(device_type_));
    std::vector<torch::Tensor> merged_idxs_greater_md;                                               
    merged_idxs_greater_md.reserve(9); 
    std::vector<torch::Tensor> still_reserved_idxs;                                                 
    still_reserved_idxs.reserve(row); 

    int slice_idx = 0, merged_vox_cnt = 0;                            
    float md_threshold = general_utils::md_thr;
    for (int i = 0; i < total_vox_num; ++i) {
        int pair_num = vox_pts_num[i];
        if (pair_num == SINGLE_POINT_VOXEL) { continue; }

        torch::Tensor md_val_slice = merged_MD.index({torch::indexing::Slice(slice_idx, slice_idx + pair_num)});
        std::tuple min_result = torch::min(md_val_slice, /*dim=*/0);
        torch::Tensor min_md_val = std::get<0>(min_result);

        torch::Tensor slice_points_idx = merged_idxs_tmp.index({torch::indexing::Slice(slice_idx, slice_idx + pair_num)});
        slice_points_idx = slice_points_idx.reshape({-1}).to(torch::kCPU);  // flatten the tensor

        std::unordered_set<int>  unique_unset(slice_points_idx.data_ptr<int>(), slice_points_idx.data_ptr<int>() + slice_points_idx.numel());
        torch::Tensor unique_tensor = torch::tensor(std::vector<int>(unique_unset.begin(), unique_unset.end()), 
                                                        torch::TensorOptions().dtype(torch::kInt32).device(torch::kCUDA));
        if (min_md_val.item<float>() < md_threshold) {
            merged_idxs_greater_md.emplace_back(unique_tensor);
            slice_idx += pair_num;
            continue;
        } else { 
            torch::Tensor min_md_idx = std::get<1>(min_result);
            int global_pair_idx = slice_idx + min_md_idx.item<int>();
            merged_idxs[merged_vox_cnt] = merged_idxs_tmp[global_pair_idx];  

            torch::Tensor mergedIdxs = merged_idxs_tmp[global_pair_idx];  
            torch::Tensor mask = torch::isin(unique_tensor, mergedIdxs);  
            torch::Tensor diff_tensor = unique_tensor.index({~mask});  
            still_reserved_idxs.emplace_back(diff_tensor);  
            slice_idx += pair_num;
            merged_vox_cnt++;
        }
    }
    merged_idxs = merged_idxs.index({torch::indexing::Slice(0, merged_vox_cnt)});

    torch::Tensor cat_merged_idxs_greater_md;
    if (!merged_idxs_greater_md.empty()) cat_merged_idxs_greater_md = torch::cat(merged_idxs_greater_md, /*dim=*/0); 
    if (cat_merged_idxs_greater_md.numel() > 0) {
        reserved_idxs = torch::cat({reserved_idxs, cat_merged_idxs_greater_md}, /*dim=*/0);  
    }

    torch::Tensor still_reservedIdxs;
    if (!still_reserved_idxs.empty()) still_reservedIdxs = torch::cat(still_reserved_idxs, /* dim= */0);
    if (merge_still_reserved && still_reservedIdxs.numel() > 0) { 
        reserved_idxs = torch::cat({reserved_idxs, still_reservedIdxs}, /*dim=*/0); 
    }

    if (merged_idxs.size(0) > 0) { MergeGaussiansInCUDA(reserved_idxs, merged_idxs, clipped_mask); }
    torch::cuda::synchronize();
}

void GaussianModel::insertPointsToVoxel(float x, float y, float z, std::size_t point_idx ) {
    Eigen::Vector3i coord = Pos2Grid(x, y, z);
    auto iter = voxel_map.find(coord);
    if (iter == voxel_map.end()) {
        voxel_map[coord] = std::vector<std::size_t>{point_idx};
    } else {
        iter->second.emplace_back(point_idx);
    }
}

void GaussianModel::traverseVoxel() {
    merged_pts_idxs.reserve(merged_pts_size);                       
    vox_pts_num.reserve(vox_pts_num_size);                          
    reserved_pt_idxs.reserve(reserved_pts_num_size);                

    for (const auto& [coord, pts_idx] : voxel_map) {
        if (pts_idx.size() == 1) {
            vox_pts_num.emplace_back(SINGLE_POINT_VOXEL);
            reserved_pt_idxs.emplace_back(pts_idx[0]);
            continue;
        }

        int cnt = 0;
        for (std::size_t i = 0; i < pts_idx.size(); ++i) {
            for (std::size_t j = i + 1; j < pts_idx.size(); ++j) {
                merged_pts_idxs.emplace_back(pts_idx[i]);
                merged_pts_idxs.emplace_back(pts_idx[j]);
                cnt += 1;
            }
        }
        vox_pts_num.emplace_back(cnt);

        if (merged_pts_idxs.size() == merged_pts_idxs.capacity()) {
            merged_pts_idxs.reserve(merged_pts_idxs.capacity() + merged_pts_size/2);
        }
        if (vox_pts_num.size() == vox_pts_num.capacity()) {
            vox_pts_num.reserve(vox_pts_num.capacity() + vox_pts_num_size/2);
        }
        if (reserved_pt_idxs.size() == reserved_pt_idxs.capacity()) {
            reserved_pt_idxs.reserve(reserved_pt_idxs.capacity() + reserved_pts_num_size/2);
        }
    }
}