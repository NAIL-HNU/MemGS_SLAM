...
void GaussianKeyframe::computeTransformTensors()
{
    if (this->set_pose_ && this->set_camera_) {
        ...
        if (!this->set_projection_matrix_) {
            ...
            //     torch::kCUDA
            // ).transpose(0, 1);
            this->projection_matrix_ = this->getProjectionMatrix2(
                this->znear_,
                this->zfar_,
                this->intr_[2],  /* cx */
                this->intr_[3],  /* cy */
                this->intr_[0],  /* fx */
                this->intr_[1],  /* fy */
                static_cast<float>(this->image_width_),
                static_cast<float>(this->image_height_),
                torch::kCUDA
            ).transpose(0, 1);
...
torch::Tensor GaussianKeyframe::getProjectionMatrix2(
    float znear,
    float zfar,
    float cx,
    float cy,
