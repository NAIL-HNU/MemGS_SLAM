...
static float init_gpu_mem = 0.0f;
...
int main(int argc, char **argv)
{
    ...
    [[maybe_unused]] auto dummy = torch::zeros({1}, torch::TensorOptions().device(torch::kCUDA));
    // Main loop
    cv::Mat im;
    ...
    std::cout << "### Gaussian Mapping Finished Iterations ###" << pGausMapper->getIteration() << std::endl;
...
void saveTrackingTime(std::vector<float> &vTimesTrack, const std::string &strSavePath)
{
    ...
    float system_fps = 1 / (totaltime / nImages);
    std::filesystem::path parent_dir = std::filesystem::path(strSavePath).parent_path();
    ...
    txt_file << "Tracking FPS: " << system_fps << std::endl;
    std::cerr << "### Tracking FPS: " << system_fps << std::endl;
...
void saveGpuPeakMemoryUsage(std::filesystem::path pathSave)
{
    ...
    std::cerr << "### Peak reserved (MB): " << max_reserved_MB << std::endl;
    std::cerr << "### Peak allocated (MB): " << max_alloc_MB << std::endl;
    std::string txt_name = "eval.txt";
    std::filesystem::path txt_path = pathSave.parent_path() / txt_name;
    std::ofstream txt_file(txt_path, std::ios::out | std::ios::app);
    if (!txt_file.is_open() || txt_file.fail())
        throw std::runtime_error("[GaussianMapper::renderAndRecordAllKeyframes]Cannot open file at " + txt_path.string());
    // txt_file << "Peak reserved (MB): " << max_reserved_MB << std::endl;
    // txt_file << "Peak allocated (MB): " << max_alloc_MB << std::endl;
    txt_file << "GPU Memory Usage Increase (MB): " << max_alloc_MB << std::endl;
}